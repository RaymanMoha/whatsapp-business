...
export function MpesaPayments({
   ...
   function progressFor(payment: Payment) {
      ...
      return [
         ...
         { label: "Receipt shared", detail: payment.receiptSharedAt ? `Shared ${formatDateTime(payment.receiptSharedAt)}` : "Ready after payment is confirmed.", done: Boolean(payment.receiptSharedAt), icon: Share2 },
      ];
   }
   ...
   return (
      <section className="overflow-hidden rounded-[28px] border border-emerald-950/10 bg-white/80 shadow-[0_24px_70px_rgba(0,63,55,.08)]">
         <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-200 px-5 py-4 sm:px-6">
            ...
            <div className="grid w-full grid-cols-1 gap-2 sm:w-auto sm:grid-cols-2"><Button type="button" variant="outline" size="sm" onClick={() => void loadPayments()} className="w-full !bg-white !text-zinc-900 hover:!bg-zinc-50"><RefreshCw className="size-4" /> Refresh</Button><Button type="button" size="sm" onClick={() => setRequestOpen((value) => !value)} className="w-full"><Send className="size-4" /> Request payment</Button></div>
         </div>
         {requestOpen ? <div className="border-b border-zinc-200 bg-emerald-50/50 p-5 sm:p-6">
            <div className="mx-auto max-w-5xl">
               ...
               <form className="grid gap-4 md:grid-cols-2 xl:grid-cols-[1.2fr_.7fr_1fr_1.5fr_auto] xl:items-end" onSubmit={startPayment}>
                  ...
                  <Button type="submit" disabled={submitting || !mpesa?.configured} className="w-full xl:w-auto">
         ...
         <div className="grid min-h-[620px] xl:grid-cols-[360px_minmax(0,1fr)]">
            ...
            {selected ? <div className="grid min-w-0 lg:grid-cols-[minmax(0,1fr)_320px]"><main className="border-b border-zinc-200 p-5 lg:border-b-0 lg:border-r sm:p-8"><div className="flex flex-wrap justify-between gap-4 border-b border-zinc-100 pb-5"><div className="min-w-0"><p className="text-xs text-zinc-400">Payment ID</p><h2 className="mt-1 break-words text-xl font-semibold">{selected.accountReference}</h2><p className="mt-1 break-words text-xs text-zinc-500">{selected.customerName || selected.phone}</p></div><div className="text-left sm:text-right"><p className="text-xs text-zinc-400">Total</p><strong className="mt-1 block text-xl text-emerald-800">KES {selected.amount}</strong></div></div><div className="py-8">{progressFor(selected).map((step, index, steps) => <div key={step.label} className="relative flex gap-4 pb-9 last:pb-0">{index < steps.length-1 ? <span className={`absolute left-[19px] top-10 h-[calc(100%-20px)] w-px ${step.done ? 'bg-emerald-600' : 'bg-zinc-200'}`} /> : null}<span className={`relative z-10 grid size-10 shrink-0 place-items-center rounded-full ${step.failed ? 'bg-red-600 text-white' : step.done ? 'bg-emerald-700 text-white' : 'border border-zinc-200 bg-white text-zinc-400'}`}><step.icon className="size-4" /></span><div className="min-w-0 pt-1"><strong className="text-sm">{step.label}</strong><p className="mt-1 text-sm leading-6 text-zinc-500">{step.detail}</p></div></div>)}</div></main><aside className="p-5 sm:p-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-zinc-400">Payment details</p><dl className="mt-4 divide-y divide-zinc-100 text-sm">{[['Phone', selected.phone], ['Channel', selected.source === 'whatsapp' ? 'WhatsApp' : 'Dashboard'], ['Receipt', selected.mpesaReceiptNumber || 'Not issued'], ['Created', formatDateTime(selected.createdAt)]].map(([label,value]) => <div key={label} className="flex justify-between gap-4 py-3"><dt className="shrink-0 text-zinc-400">{label}</dt><dd className="min-w-0 break-all text-right font-medium">{value}</dd></div>)}</dl>{selected.lineItems?.length ? <div className="mt-5 border-t pt-5"><p className="text-xs font-bold uppercase tracking-[.14em] text-zinc-400">Items</p>{selected.lineItems.map((item) => <div key={item.productId || item.name} className="flex justify-between gap-3 py-3 text-sm"><span className="min-w-0 break-words">{item.quantity} × {item.name}</span><strong className="shrink-0">KES {item.lineTotal}</strong></div>)}</div> : null}{selected.status === 'Paid' ? <div className="mt-6 grid gap-2"><Button asChild variant="outline" className="!bg-white !text-zinc-900 hover:!bg-zinc-50"><a href={`/api/commerce/payments/${encodeURIComponent(selected.id)}/receipt`} target="_blank" rel="noreferrer"><Download className="size-4" /> View receipt</a></Button><Button type="button" className="!bg-[#073f36] !text-white hover:!bg-[#052f29]" onClick={() => void shareReceipt(selected)} disabled={sharingId === selected.id}><MessageCircleMore className="size-4" />{sharingId === selected.id ? 'Sharing…' : selected.receiptSharedAt ? 'Share again' : 'Share on WhatsApp'}</Button></div> : null}</aside></div> : <div className="grid place-items-center p-10 text-sm text-zinc-500">Select a payment to inspect its progress.</div>}
         </div>
      </section>
   );
}
